...
from PySide6.QtWidgets import QMainWindow, QApplication, QTabWidget, QStatusBar, QWidget, QListWidget, QStackedWidget, QVBoxLayout, QSplitter, QToolBar, QStyle, QListWidgetItem
...
class MainWindow(QMainWindow):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        try:
            ...
            last_idx = 0
        self._sidebar.setCurrentRow(max(0, min(1, last_idx)))
        ...
        # Toolbar (Back, Save, Copy). Keep page-local buttons as-is for tests
        tb = QToolBar("Main")
        tb.setMovable(False)
        self.addToolBar(Qt.ToolBarArea.TopToolBarArea, tb)
        act_home = tb.addAction(QIcon(":/icons/home.svg"), "Back")
        act_save = tb.addAction(QIcon(":/icons/save.svg"), "Save CSV")
        act_copy = tb.addAction(QIcon(":/icons/copy.svg"), "Copy Summary")
        act_home.triggered.connect(lambda: self._sidebar.setCurrentRow(0))
        act_save.triggered.connect(lambda: getattr(self.page_ioc, "_on_save_csv", lambda: None)())
        act_copy.triggered.connect(lambda: getattr(self.page_ioc, "_on_copy_summary", lambda: None)())
        self._act_save: QAction = act_save
        self._act_copy: QAction = act_copy
    ...
    def _on_navigate(self, idx: int) -> None:
        try:
            ...
            # Enable toolbar actions only on IOC page
            enable_io = (idx == 0)
            self._act_save.setEnabled(enable_io)
            self._act_copy.setEnabled(enable_io)
        except Exception:
            pass 